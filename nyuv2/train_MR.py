...
parser.add_argument('--update_stopping', default=500, type=int)
opt = parser.parse_args()




def update_routing_masks(model):
...
class SegNet(nn.Module):
    ...
    def update_routing_mask(self, depth, new_mask, device):
        conv_block = self.get_block(depth)
        TR_layer = conv_block[2]
        TR_layer.assign_mapping(new_mask, device)
    ...
    def get_routing_mask(self, depth):
        ...
        return mapping.detach().cpu().numpy(), tested_tasks.cpu().numpy()

    def get_routing_masks(self):
        return [self.get_routing_mask(depth) for depth in range(26)]
...
nb_updates = 0
model_tested_units = [elt[1] for elt in model.get_routing_masks()]
max_updates = np.max([(elt.shape[1] - np.sum(elt, axis=1)) for elt in model_tested_units])
...
while epoch < total_epoch:
    ...
    ##### Masks updates #####
    #########################
    if epoch%opt.update_interval == 0 and epoch>=opt.update_interval and epoch<opt.update_stopping:
        if nb_updates<max_updates :
            update_routing_masks(model)
            nb_updates += 1
