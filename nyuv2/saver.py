...
class Saver():
    ...
    def __init__(self, path, args=None, metrics=None, iter=0):
        ...
        self.best_angle_mean_error = np.inf
        if args:
            ...
            self.config_dict['best_angle_mean_error'] = self.best_angle_mean_error
        ...
        self.best_angle_mean_error_weights_file = os.path.join(self.save_dir,'best_angle_mean_error_weights.pth')
    ...
    def save(self, model, iter_nb, train_metrics_values, test_metrics_values, tasks_weights=[], optimizer=None):
        ...
        if 'MEAN' in self.metrics:
            angle_mean_error = float(test_metrics_values[self.metrics.index('MEAN')])
            if angle_mean_error < self.best_angle_mean_error and iter_nb>0:
                print('Best angle mean error. Saving it.')
                torch.save(ckpt, self.best_angle_mean_error_weights_file)
                self.best_angle_mean_error = angle_mean_error
                self.config_dict['best_angle_mean_error'] = self.best_angle_mean_error
    ...
    def load(self):
        ...
        if os.path.isfile(self.config_file):
            with open(self.config_file) as f:
                ...
                self.best_angle_mean_error = prev_config['best_angle_mean_error'] if 'best_angle_mean_error' in prev_config.keys() else np.inf
