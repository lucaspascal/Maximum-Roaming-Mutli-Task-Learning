...
metrics += ['LOSS_{}'.format(c) for c in classes]
metrics += ['ACC_{}'.format(c) for c in classes]
metrics += ['PREC_{}'.format(c) for c in classes]
metrics += ['REC_{}'.format(c) for c in classes]
metrics += ['FSCORE_{}'.format(c) for c in classes]
...
while epoch<opt.total_epoch:
    ...
    for i in range(train_batches):
        ...
        data, targets = data.to(device), targets[0].to(device)
        
        # Forward pass
        feats = model(data)[0]
        logits = torch.sigmoid(feats)
        preds = (logits>=0.5).type(torch.float32)
        class_losses = torch.mean(criterion(logits, targets), 0)
        ...
        # Scoring
        with torch.no_grad():
            train_losses += class_losses.cpu() / train_batches
            train_pred += torch.sum(preds.cpu(), dim=0)
            train_to_pred += torch.sum(targets.cpu(), dim=0)
            train_well_pred += torch.sum((preds*targets).cpu(), dim=0)
            train_accs += torch.mean((preds==targets).cpu().type(torch.float32), axis=0)/train_batches
    ...
    with torch.no_grad(): 
        ...
        for i in range(val_batches):
            ...
            # Scoring
            val_losses += class_losses.cpu() / val_batches
            val_pred += torch.sum(preds.cpu(), dim=0)
            val_to_pred += torch.sum(targets.cpu(), dim=0)
            val_well_pred += torch.sum((preds*targets).cpu(), dim=0)
            val_accs += torch.mean((preds==targets).cpu().type(torch.float32), axis=0)/val_batches
